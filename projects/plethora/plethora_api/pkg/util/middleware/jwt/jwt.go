...
func (j *Service) MWFunc() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			companyID := claims["c"].(uuid.UUID)
			locationID := claims["l"].(uuid.UUID)
			username := claims["u"].(string)
			email := claims["e"].(string)
			role := plethora_api.AccessRole(claims["r"].(float64))
...
func (j *Service) GenerateToken(u *plethora_api.User) (string, string, error) {
	...
	token := jwt.NewWithClaims((j.algo), jwt.MapClaims{
		...
		"c":   u.CompanyID.String,
		"l":   u.LocationID.String,
		"exp": expire.Unix(),
	})
